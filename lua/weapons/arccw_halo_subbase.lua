...
SWEP.IsBatteryBased = true -- VHUD support

SWEP.ForceDefaultClip = 100
...
SWEP.BatteryConsumption = 1/400*100    -- amount battery is depleted per shot
...
SWEP.Hook_FireBullets = function(wep)
    if wep.ArcCW_Halo_Battery then
	    wep:SetBatteryLevel( math.Clamp( wep:GetBatteryLevel() - wep.BatteryConsumption, 0, 100 ) )
        if wep:GetBatteryLevel() <= 0 then
            wep:SetClip1(0)
        else
            wep:SetClip1(math.Round(wep:GetBatteryLevel() * 100, 0))
        end
    end
